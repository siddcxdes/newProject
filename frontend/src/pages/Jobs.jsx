...
const Jobs = () => {
    ...
    const statuses = [
        ...
        { value: 'interview', label: 'Interview', color: 'bg-sky-500' },
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-semibold text-heading mb-1">Job Search</h1>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="glass-card p-4">
                    ...
                    <p className="stat-value text-sky-400">{applications.length > 0 ? Math.round((applications.filter(a => ['interview', 'offer'].includes(a.status)).length / applications.length) * 100) : 0}%</p>
            ...
            {showAddForm && (
                <div className="glass-card p-5 space-y-4">
                    <h3 className="text-base font-semibold text-heading">New Application</h3>
            ...
            <div className="glass-card p-5">
                <h3 className="text-base font-semibold text-heading mb-4">Applications</h3>
                {applications.length === 0 ? (
                    ...
                    <div className="space-y-2">
                        {applications.map((app) => (
                            <div key={app.id} className="flex items-center justify-between p-4 bg-elevated rounded-xl border border-subtle hover:border-subtle transition-all group">
                                <div className="flex items-center gap-4">
                                    <div className={`w-2.5 h-2.5 rounded-full ${getStatusColor(app.status)}`}></div>
                                    <div>
                                        <p className="text-sm font-semibold text-heading">{app.company}</p>
                                ...
                                <div className="flex items-center gap-3">
                                    <select
                                        ...
                                        className="bg-elevated text-heading text-xs font-medium px-3 py-2 rounded-lg border border-subtle focus:outline-none focus:border-[#333]"
                                    >
                                        {statuses.map((s) => (
            ...
            <button onClick={() => logActivity('job')} className="w-full glass-card-hover p-5 text-center">
                <p className="text-sm font-semibold text-heading mb-1">Quick Log Application</p>
